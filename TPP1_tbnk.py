...
for i in range(N_faces):
    ...
    phi_prime[i] = phi_CL(XCG_droite,YCG_droite,TA,TB,DXI[i],q,GAMMA)   
...
for i in range(N_faces):
    ...
    if element_gauche != -1 and element_droite != -1:
        if element_gauche < A.shape[0] and element_droite < A.shape[1]:
            ...
            A[element_gauche, element_gauche] += flux
            A[element_droite, element_droite] += flux
            A[element_droite, element_gauche] -= flux


    if element_gauche != -1 and element_droite != -1:
        B[element_droite,0] = -S_D_CROSS[i]
        B[element_gauche,0] = S_D_CROSS[i]
        
    if element_gauche == -1 or element_droite == -1:
        
        if N1 in msh_obj.boundary_faces_nodes[1] or N1 in msh_obj.boundary_faces_nodes[3]:
            B[element_gauche,0] = S_D_CROSS[i] + D[i,0]*phi_D
        if N1 in msh_obj.boundary_faces_nodes[0] or N1 in msh_obj.boundary_faces_nodes[2]:
            B[element_gauche,0] = GAMMA*phi_prime[i]*DA[i,0]     
